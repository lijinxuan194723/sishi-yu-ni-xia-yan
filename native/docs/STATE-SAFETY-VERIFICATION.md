...
报告产物：`native210-room-reports`，artifact id `10443639970`，约 29 KB。GitHub Artifact 有保留期。

首次运行确实发现消息 `@Upsert` 对非主键唯一序号冲突没有按预期失败。保留原失败断言，改成事务内区分插入与更新、使用 ABORT 并限制身份变化后，16 项重新通过；没有删除冲突测试。

## JVM 逻辑测试：正在复核最后一轮

提交 `41df090d4939b69207436f4d1b3e189e103c38fb` 的运行 `35090425619` 执行 68 项，1 项失败，0 errors，0 skipped。失败项是 `MemoryBatchingTest.payloadBudgetIncludesJsonEscaping`。

该测试误用纯换行作为非空消息，而实现按设计排除空白消息。提交 `1e27c9483e96c6b40a8b9331ab6aa8b3af2d61ef` 在测试数据前加上正文，保留了原来的批大小、编码后大小限制、正文完整相等三项断言；同时新增数值和布尔类型摘要不能通过校验的断言。这次修改不改变生产代码。

最新 JVM 运行： https://github.com/lijinxuan194723/sishi-yu-ni-xia-yan/actions/runs/35091221487

对应最终结果尚未在本记录中确认，不能将添加测试或触发工作流表述为全部通过。

同一测试修订提交亦触发 Room 复跑： https://github.com/lijinxuan194723/sishi-yu-ni-xia-yan/actions/runs/35091221593

从 `41df090` 到 `1e27c94` 的比较仅包含该 JVM 测试文件和两份文档，生产代码与已通过的 16 项 Room 仪器测试代码均未变化。
