...
object SkillParser {
 ...
 fun safePath(path:String):Boolean=path.isNotBlank()&&path.length<=500&&!path.startsWith('/')&&!path.startsWith('\\')&&!Regex("^[A-Za-z]:").containsMatchIn(path)&&path.replace('\\','/').split('/').none{it==".."||it=="."}&&path.none{it.isISOControl()}
 fun parse(text:String,source:String,fallback:String="本地技能",files:String="[]"):SkillCandidate {
  ...
  val digest=text.sha256();val identity=if(source=="local")Normalizer.normalize(name.lowercase(java.util.Locale.ROOT),Normalizer.Form.NFC)else source
 ...
 fun zip(bytes:ByteArray):List<SkillCandidate>{
  ...
  val files=linkedMapOf<String,String>();val seen=hashSetOf<String>();var total=0;var count=0
  ZipInputStream(bytes.inputStream()).use{zip->while(true){val e=zip.nextEntry?:break;count++;require(count<=512){"ZIP 文件数量过多"};require(safePath(e.name)){"ZIP 包含不安全路径"}
   val path=Normalizer.normalize(e.name.replace('\\','/'),Normalizer.Form.NFC)
   require(seen.add(path)){"ZIP 路径重复"}
  ...
  val candidates=names.map{path->val parent=path.substringBeforeLast('/',"");val refs=files.filter{(k,v)->k!=path&&k.startsWith(if(parent.isEmpty())"" else "$parent/")&&v.isNotEmpty()}.entries.take(20)
  ...
  require(candidates.map{it.skill.id}.distinct().size==candidates.size){"ZIP 包含同名技能，请分别导入并确认更新"}
  return candidates
 }
}
class SkillInstaller(private val context:Context,private val http:Http,private val dao:LukeDao){
 ...
 suspend fun install(candidate:SkillCandidate,expectedDigest:String?=null){
  require(candidate.skill.content.sha256()==candidate.skill.digest){"技能内容校验失败"}
  check(dao.installReviewedSkill(candidate.skill,expectedDigest)){"技能已存在、已更新或已卸载，请重新查看内容后确认"}
 }
 suspend fun setEnabled(id:String,enabled:Boolean,expectedDigest:String?=null){
  val skill=dao.skill(id)?:error("技能不存在")
  val reviewed=expectedDigest?:if(!enabled)skill.digest else error("启用前请先查看并确认这个版本的技能")
  require(!enabled||skill.content.sha256()==skill.digest){"技能内容校验失败"}
  check(dao.setSkillEnabledIfUnchanged(id,enabled,reviewed)==1){"技能内容已改变或已卸载，请重新查看"}
 }
