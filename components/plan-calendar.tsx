...
export function PlanCalendar({ month, onMonth, today, selected, onSelect, data }: Props) {
 ...
 const holidayState=useHolidayYear(year,m);
  ...
  return <section className="card calendar luke-plan-calendar" aria-label="和夏彦的一起计划月历">
    ...
    <div className="calendar-grid">
      ...
      {Array.from({ length: new Date(year, m + 1, 0).getDate() }, (_, i) => {
        const day = dateKey(new Date(year, m, i + 1)), isToday = day === today,holiday=holidayState.days[day]??null;
    ...
    </div><div className="calendar-tools"><DateField aria-label="跳转到指定日期" allowClear={false} value={selected} onValueChange={day=>{onSelect(day);onMonth(new Date(day.slice(0,7)+'-01T12:00:00'));}}/><small>{holidayState.complete?'休 · 放假　班 · 调休上班':Object.keys(holidayState.days).length?'仅收录部分跨年日期':'此年份尚未取得完整调休安排'}</small></div>
  </section>;
}
