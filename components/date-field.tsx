'use client';
import {useHolidayYear} from '@/hooks/use-holidays';
...
import {parseCalendarDate} from '@/lib/china-holidays';
import {normalizeDateEntry,yearFromEntry,boundedMonth,moveCalendarDay} from '@/lib/date-interaction';
type Props={compact?:boolean;value:string;onValueChange:(value:string)=>void;min?:string;max?:string;disabled?:boolean;allowClear?:boolean;className?:string;'aria-label'?:string};
export function DateField({value,onValueChange,min='1900-01-01',max='2100-12-31',disabled=false,allowClear=true,className='',compact=false,...props}:Props){
 ...
 const holidayState=useHolidayYear(year,m,open);
 const selectedState=useHolidayYear(Number(draft.slice(0,4))||year,undefined,false);
 const selectedHoliday=selectedState.days[draft]??null;
 ...
 return <><button ref={trigger} type="button" className={`date-field ${className}`} disabled={disabled} aria-label={props['aria-label']??'选择日期'} aria-haspopup="dialog" aria-expanded={open} onClick={start}><span>{value?value.replaceAll('-',compact?'/':' / '):'选择日期'}</span><CalendarDays size={18}/></button>
 <Dialog open={open} onOpenChange={setOpen}><DialogContent className="date-picker date-picker-refined" initialFocus={kind=>kind==='keyboard'?(document.getElementById(`${id}-${focusDay}`)??false):false} finalFocus={trigger}>
  <div className="date-picker-body">
   ...
   <div className="date-grid" role="group" aria-label={`${year}年${m+1}月`}>{'一二三四五六日'.split('').map(day=><small key={day}>{day}</small>)}{Array.from({length:(new Date(year,m,1).getDay()+6)%7},(_,index)=><span key={`pad-${index}`}/>)}{Array.from({length:days},(_,index)=>{
    const day=dateKey(new Date(year,m,index+1)),holiday=holidayState.days[day]??null;
   ...
   <p id={`${id}-status`} className="date-context" role="status">{error||(!parseCalendarDate(draft)?'请输入完整日期':!allowed(draft)?`可选范围：${lower} 至 ${upper}`:selectedHoliday?`${selectedHoliday.name} · ${selectedHoliday.kind==='rest'?'放假':'调休上班'}`:!holidayState.complete?'此年份尚未收录完整放假安排':' ')}</p>
  </div>
