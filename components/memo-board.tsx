'use client';
import {useMemoPersistence} from '@/hooks/use-memo-persistence';
import {formatMemo} from '@/lib/memo-format';
...
export function MemoBoard({legacyNotes=[],draftText='',onDraftUsed}:{legacyNotes?:LegacyNote[];draftText?:string;onDraftUsed?:()=>void}){
 ...
 function allowWrite(){if(!blocked.current)return true;if(!window.confirm('原笔记数据异常。继续会用新的笔记数据覆盖它，建议先导出原始数据。仍要继续吗？'))return false;blocked.current=false;rawBackup.current=localStorage.getItem(MEMO_STORAGE_KEY);return true;}
 ...
 function insert(before:string,after=''){if(!active||preview)return;const el=bodyRef.current,start=el?.selectionStart??active.body.length,end=el?.selectionEnd??start;const change=formatMemo(active.body,start,end,before,after);if(change.body.length>100000){setStatus('正文已达到字数上限');return;}mutate(active.id,{body:change.body});requestAnimationFrame(()=>{bodyRef.current?.focus();bodyRef.current?.setSelectionRange(change.start,change.end);});}
 ...
 return <div ref={rootRef} className={styles.shell}>
  ...
  {!active&&<div className={styles.listView}>
   <header className={styles.heroHead} data-memo-head210><div><span>LUKE · LITTLE MEMORIES</span><h2>我们的手记</h2><p role="status">{counts.all} 条珍藏 · {status}</p></div><div className={styles.headActions} data-memo-redundant210><button aria-label="搜索笔记" onClick={()=>document.getElementById('memo-search')?.focus()}><Search/></button><button aria-label="笔记本" onClick={()=>setNotebooksOpen(true)}><BookHeart/></button></div></header>
   ...
   <p className={styles.listHint} data-memo-hint210>点开继续写，长按或点「···」整理。</p><label className={styles.searchBar}><Search size={17}/><input id="memo-search" value={query} maxLength={120} onChange={e=>setQuery(e.target.value)} aria-label="搜索手记" placeholder={`搜索${categoryLabel}`}/>{query&&<button aria-label="清空搜索" onClick={()=>setQuery('')}>×</button>}</label>
   ...
   {!notebooksOpen&&!selecting&&!actionNote&&createPortal(<button className={styles.fab} data-memo-fab210 aria-label="新建笔记" onClick={addNote}><Plus size={30}/></button>,document.body)}
  </div>}
  ...
  {createPortal(<MotionPresence present={!!active} kind="sheet">{active&&<article ref={editorRef} data-memo-editor="true" className={styles.editorView}>
   ...
   <div className={styles.editorBottom} data-memo-bottom210><div className={styles.editorFootnote}><span>{preview?"阅读模式":"正在珍藏这一刻"}</span><span>{active.body.length.toLocaleString()} 字符</span></div><MotionPresence present={moodOpen}>{moodOpen&&<div className={styles.moodPicker}><button data-active={!active.mood} onClick={()=>{mutate(active.id,{mood:undefined});setMoodOpen(false);}}>无</button>{memoMoods.map(mood=><button key={mood} data-active={active.mood===mood} onClick={()=>{mutate(active.id,{mood});setMoodOpen(false);}}>{mood}</button>)}</div>}</MotionPresence><div className={styles.quickBar} data-memo-toolbar210><img src="/images/companions/toast.webp" alt="" width={28} height={28}/><button className={styles.moodButton} data-on={!!active.mood} onClick={()=>setMoodOpen(v=>!v)}><Heart size={16}/>{active.mood??'心情'}</button><span/><button aria-label="小标题" title="小标题" onClick={()=>insert('## ')}>H₂</button><button aria-label="项目列表" title="项目列表" onClick={()=>insert('- ')}>≡</button><button aria-label="待办列表" title="待办列表" onClick={()=>insert('- [ ] ')}>☑</button><button aria-label="加粗" title="加粗" onClick={()=>insert('**','**')}>B</button><button aria-label="引用段落" title="引用段落" onClick={()=>insert('> ')}>❞</button></div></div>
