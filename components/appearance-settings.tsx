...
export function AppearanceSettings() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved && isStoredBackground(saved)) {
  ...
  async function choose(file: File) {
    ...
    try {
      ...
      setBackground(value); setNotice('新背景已保存，仅存储在本机。');
    } catch (error) {
      if (!active.signal.aborted && selection.current === active) {
        setBackgroundError(error instanceof Error ? error.message : '图片处理失败，原背景已保留。');
      }
    } finally {
  ...
  function reset() {
    ...
    try {
      saveChatBackground(localStorage, null);
  ...
  return <div className="settings-stack">
    <div className="appearance-preview" aria-busy={!scene}>
      {scene ? <><SeasonPhoto season={scene.season} /><Ambience scene={scene} /></> : <span role="status">正在准备外观…</span>}
    </div>
    <fieldset className="appearance-options"><legend>季节主题</legend><div>
      {(['auto', ...seasons] as const).map((value, index) => <button type="button" key={value} disabled={!scene}
        aria-pressed={options.season === value} onClick={() => setOptions({ ...options, season: value })}>
        {['随日期', '春', '夏', '秋', '冬'][index]}</button>)}
    </div></fieldset>
    <fieldset className="appearance-options"><legend>昼夜光照</legend><div>
      {(['auto', ...periods] as const).map(value => <button type="button" key={value} disabled={!scene}
        aria-pressed={options.period === value} onClick={() => setOptions({ ...options, period: value })}>
        {value === 'auto' ? '随时间' : value}</button>)}
    </div></fieldset>
    <label className="inline-label"><input type="checkbox" role="switch" aria-checked={options.effects !== false}
      checked={options.effects !== false} onChange={event => setOptions({ ...options, effects: event.target.checked })} /> 季节动画</label>
    <section className={styles.background} aria-label="悄悄话背景设置" aria-busy={busy}>
      <h3>悄悄话背景</h3>
      <p className={styles.help} id={helpId}>支持 JPG、PNG、WebP，最大 8 MB。自动优化图片尺寸，节省本地空间，不上传图片。</p>
      ...
      <div className={styles.actions}>
        <label className={styles.upload}><ImagePlus size={17} aria-hidden="true" /><span>{busy ? '正在优化图片…' : background ? '更换背景图片' : '选择背景图片'}</span>
          <input ref={input} type="file" accept="image/jpeg,image/png,image/webp" aria-label="选择悄悄话背景图片" aria-describedby={helpId}
