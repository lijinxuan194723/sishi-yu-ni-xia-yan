...
export function MemoActions({note,folders,onAction,onDone}:{note:MemoDocument;folders:{id:string;name:string}[];onAction:(action:MemoAction)=>void;onDone:()=>void}){
 ...
 const popup=useRef<HTMLDivElement>(null),previousPage=useRef(page),returnTo=useRef<HTMLElement|null>(null);
 useLayoutEffect(()=>{
  if(previousPage.current===page)return;
  previousPage.current=page;
  if(page==='main'){
   const target=popup.current?.querySelector<HTMLButtonElement>(`[data-memo-step="${returnTo.current?.dataset.memoStep??''}"]`);
   target?.focus({preventScroll:true});
  }else{
   popup.current?.querySelector<HTMLElement>(page==='trash'?'[data-safe-cancel]':'[data-memo-step-title]')?.focus({preventScroll:true});
  }
 },[page]);
 function enter(next:'move'|'trash',source:HTMLButtonElement){returnTo.current=source;setPage(next);setFailure('');panel.current?.scrollTo({top:0});}
 function finalFocus(){
  if(pending.current?.kind==='edit')return false;
  if(origin.current?.isConnected&&!origin.current.closest('[inert]'))return origin.current;
  return [...document.querySelectorAll<HTMLButtonElement>('[data-memo-id]')].find(node=>node.dataset.memoId===note.id)??false;
 }
 ...
 return <Dialog open={open} onOpenChange={setOpen} onOpenChangeComplete={visible=>{
  ...
  try{if(action)onAction(action);onDone();}
  catch(error){choosing.current=false;setFailure(error instanceof Error?error.message:'操作未完成，请重试。');setOpen(true);}
 }}>
  <DialogContent ref={popup} finalFocus={finalFocus} data-memo-actions="true" data-action-step={page} className={styles.sheet} overlayClassName={styles.backdrop} onKeyDownCapture={event=>{if(event.key==='Escape'&&page!=='main'){event.preventDefault();event.stopPropagation();goBack();}}}>
   <header className={styles.sheetHeader}>
    ...
    <DialogTitle data-memo-step-title tabIndex={-1}>{page==='trash'?'移到回收站？':page==='move'?'放进哪本手记？':memoDisplayTitle(note)}</DialogTitle>
   ...
   <div ref={panel} className={styles.sheetBody} inert={!open}>
    ...
    {page==='main'?<>
     ...
     <div className={styles.actionGroup}>
      <button data-memo-step="move" onClick={event=>enter('move',event.currentTarget)}><Folder size={18}/><span>移动到笔记本</span><ChevronRight size={16}/></button>
     ...
     <button className={styles.dangerAction} data-memo-step="trash" onClick={event=>enter('trash',event.currentTarget)}><Trash2 size={18}/>移到回收站</button>
     ...
     </div>:<div className={styles.confirmBox}><p>“{memoDisplayTitle(note)}”</p><button className={styles.dangerAction} data-haptic="confirm" onClick={()=>choose({kind:'trash'})}><Trash2 size={18}/>确认移到回收站</button><button data-safe-cancel className={styles.cancel} onClick={goBack}>保留手记</button></div>}
   </div>
