...
def verify_settings_calendar(page, check, out):
    ...
    back();check('settings search survives returning from a result',page.get_by_label('搜索设置',exact=True).input_value()=='聊天模型');page.get_by_role('button',name='清除设置搜索',exact=True).click();section('天气与位置')
    ...
    page.get_by_role('button',name='撤销本次修改',exact=True).click();back();section('日常与数据')
