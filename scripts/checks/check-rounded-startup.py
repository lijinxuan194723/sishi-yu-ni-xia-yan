...
check('single system splash on API31+, release gated by prepared frame','PlatformSplash.install(MainActivity.this,this)' in java and 'hold=()->released;' in java and 'boolean wasHeld=!released;' in java)
check('predraw hold removed during disposal','removeOnPreDrawListener(hold)' in java and 'disposed=true;handler.removeCallbacksAndMessages(null);releaseDraw();' in java)
