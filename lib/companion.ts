import {parseConversations,type Conversations} from './chat-sessions.ts';
...
export function parseData(text:string):Data{
 ...
 const conversations=p.conversations===undefined?undefined:parseConversations(p.conversations);
 if(p.messages.some((m:any)=>m.conversationId!==undefined&&(!conversations||!conversations.items.some(c=>c.id===m.conversationId))))throw Error('消息所属对话缺失，原始记录未覆盖。');
