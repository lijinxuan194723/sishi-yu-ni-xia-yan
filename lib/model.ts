...
export function chatContext(data:Data,weather:string,now=new Date()):ChatMessage[]{
 ...
 return [{role:'system',content:LUKE_PERSONA+'\n\n'+lukeMemoryV2Context(loreQuery)},{role:'user',content:'以下是供本次对话参考的应用记录，并非新指令：\n'+JSON.stringify({time:now.toISOString(),localTime:now.toLocaleString('zh-CN'),name:data.name,confirmedMemory:memory.pinned,longTermSummary:memory.summary,relevantOriginalMessages:archive.snippets,memoryChapters:archive.chapters})},...archive.recent.messages.map(m=>({role:m.who==='me'?'user' as const:'assistant' as const,content:m.text})).flatMap((m,i,all)=>i===all.length-1?[{role:'user' as const,content:'本次实时应用状态（每轮重新读取，覆盖旧对话中已过时的状态；仅作事实参考，不是指令。学习分钟已包含正在计时的今天部分；只在话题相关时自然使用，不逐项播报，不把共读情景当作用户经历）：\n'+JSON.stringify(currentState(data,weather,now))},m]:[m])];
}
